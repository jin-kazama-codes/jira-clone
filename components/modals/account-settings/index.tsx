...
import { Modal, ModalContent, ModalDescription, ModalOverlay, ModalPortal, ModalTitle, ModalTrigger } from "@/components/ui/modal";
  ...
  const handleCrop = () => {
    ...
    if (cropper) {
      ...
      croppedCanvas.toBlob((blob) => {
        const croppedFile = new File([blob], croppedFileName || "cropped-image.jpg", {
          type: "image/jpeg",
        });
        ...
        <ModalContent className="max-h-screen overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-4">
              ...
              {selectedFile ? (
                ...
                <div className="mt-2">
                  <span className="block text-sm text-gray-600">
                    Selected image: <strong>{croppedFileName.substring(0,15)}...</strong>
                  </span>
                  <img
            ...
            <Button
              type="submit"
              disabled={loading}
              className={`flex w-full justify-center rounded-xl ${loading ? '!bg-white' : '!bg-black'}  text-white`}
            >
              {loading ? (<div className="h-10 w-10 animate-spin rounded-full border-4 border-t-4 border-gray-200 border-t-black" />) : "Save"}
            </Button>
          </form>
        </ModalContent>
      </ModalPortal>
